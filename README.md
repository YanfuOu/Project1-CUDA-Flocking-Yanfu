...
![50k boids simulation](media/Proj1-50k-boids-sim.gif)
...
For all there implementation, there seems to be an idea block size where the FPS is maximized. Any block size bigger or smaller than that results in a somewhat less ideal performance. I think it's because too small of a block size results in too many unneccessary division across the SM, resulting in a lot of overhead and missed opportunties to use shared cache. Too big of a block size results in register overflow, forcing the threads to utilize local or even global memory as a backup, also leading to performance degredation.   

| block size(# threads) | Native(FPS) | Scattered Uniform Grid(FPS) | Coherent Grid(FPS) |
| ---------- | ------ | ---------------------- | ------------- |
